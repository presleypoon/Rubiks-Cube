...
class faces {
	...
	GetColour() {
		let SingleLetterColour = [
			this.U[0], this.U[3], this.U[1],
			this.U[6], this.U[4], this.U[2],
			this.F[0], this.U[7], this.U[5],
			this.R[2], this.F[3], this.F[1],
			this.U[8], this.R[1], this.R[5],
			this.F[6], this.F[4], this.F[2],
			this.R[0], this.R[4], this.R[8],
			this.F[7], this.F[5], this.R[3],
			this.R[7], this.F[8], this.R[6]
		];
		...
		for (let i = 0; i < SingleLetterColour.length; i++) {
			...
			switch (temp) {
				...
				default: {
					if (dev) color.push("");
					else throw new Error(`Invalid decoding at index ${i} and char ${temp}`);
				}
			}
		}
	}
...
try {
	...
	for (i = 0; i < Move.length; i++)  Move[i].disabled = true;
	for (i = 0; i < Submit.length; i++)  Submit.disabled = true;
}